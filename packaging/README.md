...
`metanc/orocos` Prefix upload channel. Building and testing do not upload
...
Publication is additionally restricted to `OptimalCNC/rock-orocos`. A
published, non-prerelease release in that repository is the only publication
trigger.

Configure Prefix once before publishing the first release:

1. Open `https://prefix.dev/channels/@metanc/orocos`, then select Repository
   Access (formerly Trusted Publishers).
2. Add the GitHub repository `OptimalCNC/rock-orocos` and the exact workflow
...
`https://prefix.dev/metanc/orocos`. A downstream workspace normally
