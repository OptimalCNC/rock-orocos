...
if File.file?(dockerfile)
  ...
  errors << "Dockerfile must build Orocos/Rock in a builder stage" unless contents.match?(/^FROM optimalcnc\/metanc:latest AS builder$/)
  errors << "Dockerfile final image must start again from MetaNC shared image" unless contents.match?(/^FROM optimalcnc\/metanc:latest(?: AS final)?$/)
  ...
  final_stage = contents.split(/^FROM optimalcnc\/metanc:latest(?: AS final)?$/).last || ""
