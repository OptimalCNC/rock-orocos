...
if File.file?(dockerfile)
  ...
  errors << "Dockerfile must default to the MetaNC base image" unless contents.match?(/^ARG METANC_BASE_IMAGE=optimalcnc\/metanc:base$/)
  errors << "Dockerfile must build Orocos/Rock from the configurable MetaNC base image" unless contents.match?(/^FROM \$\{METANC_BASE_IMAGE\} AS builder$/)
  errors << "Dockerfile final image must start again from the configurable MetaNC base image" unless contents.match?(/^FROM \$\{METANC_BASE_IMAGE\} AS final$/)
  errors << "Dockerfile must not use the final MetaNC image as an Orocos/Rock base" if contents.include?("optimalcnc/metanc:latest")
