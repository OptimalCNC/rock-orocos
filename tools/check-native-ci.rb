...
if !File.file?(workflow_path)
  ...
  errors << "native CI must build gnulinux through the wrapper" unless contents.include?('./tools/install.sh --prefix "$OROCOS_PREFIX" --target gnulinux')
  errors << "native CI must validate the gnulinux installed prefix" unless contents.include?('./tools/validate-install.sh --prefix "$OROCOS_PREFIX" --target gnulinux')
  errors << "native CI must fail on compiler warnings" unless contents.include?("compiler warning budget exceeded")
  errors << "native CI must use a fresh /tmp/orocos install prefix" unless contents.include?("OROCOS_PREFIX: /tmp/orocos")
  errors << "native CI must not use the clean-room Docker /opt/orocos install prefix" if contents.include?("OROCOS_PREFIX: /opt/orocos")
