...
unless File.file?(workflow_path)
  ...
  errors << "Windows package CI must use the canonical Prefix channel" unless workflow.dig("env", "PREFIX_CHANNEL") == "metanc/orocos"
  errors << "Windows package CI channel must not contain @" if workflow.dig("env", "PREFIX_CHANNEL").to_s.include?("@")
  unless workflow.dig("env", "PUBLIC_CHANNEL_URL") == "https://prefix.dev/metanc/orocos"
  ...
  expected_repository_name = "OptimalCNC/rock-orocos"
...
unless File.file?(recipe_path)
  ...
  expected_repository = "https://github.com/OptimalCNC/rock-orocos"
  ...
  unless recipe.include?("    - OptimalCNC")
    errors << "Windows package recipe must identify OptimalCNC as maintainer"
  end
  [
...
unless File.file?(exporter_path)
  ...
  unless boundary_membership_tokens.all? { |token| runtime_batch&.include?(token) } &&
         runtime_batch&.scan(membership_source)&.size == 4
...
unless File.file?(staging_path)
  ...
  unless staging.include?('[string]$Channel = "metanc/orocos"')
