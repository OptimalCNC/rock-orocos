...
[`examples/pixi-consumer`](https://github.com/OptimalCNC/rock-orocos/tree/main/examples/pixi-consumer)
...
```toml
[workspace]
channels = ["https://prefix.dev/metanc/orocos", "conda-forge"]
...
Runtime-only users replace `orocos-dev = "==0.1.11"` with
`orocos = "==0.1.11"` under `[dependencies]`.
