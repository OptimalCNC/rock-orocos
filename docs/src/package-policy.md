...
| `eigen_typekit` | RTT support for Eigen vectors and matrices | `OptimalCNC/rtt_geometry`, pinned `dev` revision; only the `eigen_typekit/` package |
...
| `cpp-httplib` | Private HTTP/TLS listener dependency | `OptimalCNC` maintenance fork with owned sockets, signal policy, and exact request routing |
| `rtt_http` | Independent REST object model, JSON codec SDK, and operation executor | `OptimalCNC` upstream |
