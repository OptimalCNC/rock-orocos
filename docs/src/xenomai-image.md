...
[recovery reference](https://github.com/OptimalCNC/rock-orocos/blob/main/images/publish/README.md)
...
After publication succeeds, dispatch `xenomai-tests.yml` from `main`. Supply
the AMI ID from that publication's `published-image.json`:

```bash
gh workflow run xenomai-tests.yml --repo OptimalCNC/rock-orocos --ref main \
  -f expected_ami_id=ami-REPLACE_WITH_PUBLISHED_ID
```

The workflow uses the `rock-orocos-xenomai` selector in `.github/runs-on.yml`,
which selects the newest matching image owned by the RunsOn AWS account.
Before testing, it checks the running instance's AMI against the supplied ID
and records its EC2 identity and image manifest. A mismatch fails acceptance.
The source revision must match the workflow checkout unless `image_revision`
explicitly selects an older publication. See the
[RunsOn custom-image reference](https://runs-on.com/docs/runners/custom/).

Each dispatch starts an on-demand
[`m7i.xlarge`](https://aws.amazon.com/ec2/instance-types/m7i/) instance with four CPUs,
16 GiB RAM and a 64 GiB root volume. The 50-minute job limit leaves room
within the installation's 60-minute runner lifetime. The default RunsOn
environment is `production` in `us-east-1`; this differs from the
...
before migrating a downstream workflow.
