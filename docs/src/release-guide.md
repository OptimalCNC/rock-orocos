...
- a release commit reachable from the freshly fetched protected
  `main` branch;
- repository identity `OptimalCNC/rock-orocos`;
...
In the `metanc/orocos` channel's Repository Access settings,
authorize:

- repository `OptimalCNC/rock-orocos` with workflow
...
6. Confirm clean runtime and development installs pass through
   `https://prefix.dev/metanc/orocos`.
