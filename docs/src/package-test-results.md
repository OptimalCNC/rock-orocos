...
| `ocl-integration` | `deploy`, `testlogging`, `report`, `tcpreport`, `ncreport`, and `testWithStateMachine` | Passes locally after qualifying the timer state-machine test's Boost pointer type in `liufang-robot/ocl` `MetaNC` merge commit `91739f7`: 6/6 CTest cases. |
